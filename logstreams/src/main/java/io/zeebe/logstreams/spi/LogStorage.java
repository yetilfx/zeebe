...
public interface LogStorage {
  /**
   * Status code returned by the {@link StorageReader#read(ByteBuffer, long)} operation in case the
   * provided address is invalid and does not exist.
   */
  long OP_RESULT_INVALID_ADDR = -1L;

  /**
   * Status code returned by the {@link StorageReader#read(ByteBuffer, long)} operation in case the
   * provided address does exist but data is not available yet. This indicates that retrying the
  ...
  /**
   * Status code returned by the {@link StorageReader#read(ByteBuffer, long)} operation only if
